...
1. Crie uma cópia do arquivo de modelo [.env.example](file:///D:/imersao-alura/backend/.env.example) e renomeie-a para `.env`:
   ```bash
...
Abaixo estão listadas as rotas expostas em [main.py](file:///D:/imersao-alura/backend/main.py):

### 🏠 Páginas e Auxiliares
* **`GET /`**: Serve o frontend [index.html](file:///D:/imersao-alura/frontend/index.html) diretamente a partir do backend.
...
* O arquivo `.env` que armazena a chave criptográfica **nunca** é enviado ao GitHub. Ele está devidamente listado no arquivo [.gitignore](file:///D:/imersao-alura/.gitignore).
